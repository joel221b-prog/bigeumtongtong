 ...
 * 환경변수:  KOMSA_API_KEY (기상청·KOMSA 공통 공공데이터 키)
 *
 * 비금도 격자 좌표 (Lambert 공식 계산):
 *   위도 34.50°N, 경도 126.01°E → nx=44, ny=61
 */

const NX = 44;  // 비금도 격자 X
const NY = 61;  // 비금도 격자 Y
const BASE_URL = "https://apis.data.go.kr/1360000/VilageFcstInfoService_2.0/getVilageFcst";

/* KST 기준 발표 시간 계산 (02/05/08/11/14/17/20/23시) */
function getBaseDateTime() {
  ...
  let bt = 23;
  for (const t of BASES) { if (h >= t) bt = t; }

  if (h < 2) {                          // 자정~01시 → 전날 23시 발표분
...
exports.handler = async () => {
  const API_KEY = process.env.KOMSA_API_KEY;  // 공공데이터 공통 키
  ...
  try {
    ...
    catch { return res(502, { error: "기상청 JSON 파싱 실패", raw: text.slice(0, 200) }); }
    ...
    console.log("[weather] 일별 데이터:", daily.length, "일 / 현재기상:", current);
